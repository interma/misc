...
# interma added it
def ssh_lsof(host, port):
    # cmd = "ssh gpadmin@%s 'sudo /usr/sbin/lsof -i | grep %d'" % (host, port)
    cmd = "ssh gpadmin@%s 'sudo /usr/sbin/lsof -nP -iTCP:%d'" % (host, port)
    r = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=None)
    r.wait()
    print("\tcmd => "+ cmd)
    for line in r.stdout.read().decode().split('\n'):
        print("\t"+line)
...
def print_ipv4_event(cpu, data, size):
    ...
    printb(b"%-7d %-12.12s %-2d %-16s %-5d %-16s %-5d" % (event.pid,
        ...
        event.lport))
    dhost = inet_ntop(AF_INET, pack("I", event.daddr))
    ssh_lsof(dhost, event.dport)
...
def print_ipv6_event(cpu, data, size):
    ...
    printb(b"%-7d %-12.12s %-2d %-16s %-5d %-16s %-5d" % (event.pid,
        ...
        event.lport))
    dhost = inet_ntop(AF_INET6, event.daddr)
    ssh_lsof(dhost, event.dport)
