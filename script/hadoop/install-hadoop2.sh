...
install()
{
	...
	pdsh -w ^nn_host "sudo mkdir -p $NN_DATA_DIR && sudo chown $USER:$GROUP $NN_DATA_DIR"
	pdsh -w ^snn_host "sudo mkdir -p $SNN_DATA_DIR && sudo chown $USER:$GROUP $SNN_DATA_DIR"
	pdsh -w ^dn_hosts "sudo mkdir -p $DN_DATA_DIR && sudo chown $USER:$GROUP $DN_DATA_DIR"

	echo "Creating log directories on all hosts..."
	pdsh -w ^all_hosts "sudo mkdir -p $YARN_LOG_DIR && sudo chown $USER:$GROUP $YARN_LOG_DIR"
	pdsh -w ^all_hosts "sudo mkdir -p $HADOOP_LOG_DIR && sudo chown $USER:$GROUP $HADOOP_LOG_DIR"
	pdsh -w ^all_hosts "sudo mkdir -p $HADOOP_MAPRED_LOG_DIR && sudo chown $USER:$GROUP $HADOOP_MAPRED_LOG_DIR"

	echo "Creating pid directories on all hosts..."
	pdsh -w ^all_hosts "sudo mkdir -p $YARN_PID_DIR && sudo chown $USER:$GROUP $YARN_PID_DIR"
	pdsh -w ^all_hosts "sudo mkdir -p $HADOOP_PID_DIR && sudo chown $USER:$GROUP $HADOOP_PID_DIR"
	pdsh -w ^all_hosts "sudo mkdir -p $HADOOP_MAPRED_PID_DIR && sudo chown $USER:$GROUP $HADOOP_MAPRED_PID_DIR"
	...
	put_config --file mapred-site.xml --property yarn.app.mapreduce.am.staging-dir --value /root
	...
	hadoop fs -chown -R $USER:$GROUP /root
	hadoop fs -chmod -R g+rwx /root
	...
	hadoop fs -chown -R $(whoami):$(whoami) /user/$(whoami)
